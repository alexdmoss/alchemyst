...
def redir_pdfs():
    ...
    if doc_id is not None:
        ...
        if group == "category":
            return redirect(
                url_for("display_notes_by_category", category=category.lower()),
                code=301,
            )
        elif group == "level":
            if category == "1":
                return redirect(
                    url_for("display_notes_by_category", category="first-year"),
                    code=301,
                )
            elif category == "2":
                return redirect(
                    url_for("display_notes_by_category", category="second-year"),
                    code=301,
                )
            elif category == "3":
                return redirect(
                    url_for("display_notes_by_category", category="third-year"),
                    code=301,
                )
            else:
